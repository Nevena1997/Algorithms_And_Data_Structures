...
{
    //Izlaz iz rekurzije je kada dodjemo do jednoclanog segmenta, tj. lista stabla
    //Tada znamo da smo na poziciji na kojoj treba upisati novu vrednost
    if (x == y) {
    ...
    //Ukoliko je trazeni indeks i manji ili jednak od sredisnjeg elementa segmenta, nastavljamo da azuriramo levo podstablo
    if (index <= middle)
        update_tree(segment_tree, 2 * k, x, middle, index, new_value);
    //Ukoliko je trazeni indeks i veci od sredisnjeg elementa segmenta, nastavljamo da azuriramo desno podstablo
    else
        update_tree(segment_tree, 2 * k + 1, middle + 1, y, index, new_value);

    //Azuriramo vrednost roditelja na osnovu azuriranih vrednost levog i desnog sina
    segment_tree[k] = segment_tree[2 * k] + segment_tree[2 * k + 1];
}
...
int main ()
{
    ...
    update_tree(segment_tree, 1, 0, n - 1, i, v);
